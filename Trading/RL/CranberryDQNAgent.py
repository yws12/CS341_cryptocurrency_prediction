'''
Cranberry
1. Include additional internal states into features: 
    1) portfolio.starting_cash
    2) steps_left # how many steps away we are from the episode end
    3) (normalized) last_buy_price # maintained for each episode and set to env.getCurrentPrice everytime action == Action.BUY
    4) 
2. Can disable "xaction_fee" in test() to see impact of transaction fee 
'''
...
#         reward = v_end / v_0 * 100
        ...
        for state, action, next_state, is_last_step, v_t, v_t_plus1 in minibatch:
