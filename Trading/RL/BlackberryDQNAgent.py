'''
Blackberry:
1. If cannot buy/sell, choose second best option.
2. Prioritized experience relay: sample high return memory more often. self.memory is a sorted list of (priority, record) where 'record' is (state, action, next_state, is_last_step, v_t, v_t_plus1). The parameter alpha in choosing record to use is set to 0.02. This can be tuned. alpha=0 is uniform distribution. Larger alpha is sampling high reward records more often. See: http://www0.cs.ucl.ac.uk/staff/d.silver/web/Applications_files/prioritized-replay.pdf, page 4.
'''
...
#         reward = v_end / v_0 * 100
        ...
        for priority, record in minibatch:
            ...
            if reward_func == 'Andy':
                reward = (v_t_plus1 / self.initial_cash - 1) * 100 # immediate reward, first term in Bellman Eq
            else:
...
#         self.memory = [] # clear memory after using this episode
    ...
    # Agent Training    
    def train(self, experiment_name, session, start_time, end_time, episode_len=100, num_episodes=100, verbose=True, auto_save_and_load=True, save_every=50, reward_func='Andy'):
        ...
        for i in range(current_episode, num_episodes):
            ...
            ep_reward = (v_t_plus1 / self.initial_cash - 1) * 100
